...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="rico",
        choices=["rico", "publaynet", "magazine", "visily"],
        help="dataset name",
    )
    ...
    # General
    parser.add_argument("--latent_size", type=int, default=4, help="latent size")
    ...
    # Generator
    parser.add_argument(
        "--G_d_model", type=int, default=256, help="d_model for generator"
    )
    parser.add_argument("--G_nhead", type=int, default=4, help="nhead for generator")
    parser.add_argument(
        "--G_num_layers", type=int, default=8, help="num_layers for generator"
    )

    # Discriminator
    parser.add_argument(
        "--D_d_model", type=int, default=256, help="d_model for discriminator"
    )
    parser.add_argument(
        "--D_nhead", type=int, default=4, help="nhead for discriminator"
    )
    parser.add_argument(
        "--D_num_layers", type=int, default=8, help="num_layers for discriminator"
    )
    ...
    for epoch in range(max_epoch):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            padding_mask = ~mask
            z = torch.randn(
                label.size(0), label.size(1), args.latent_size, device=device
            )
            ...
            D_real, logit_cls, bbox_recon = netD(
                bbox_real, label, padding_mask, reconst=True
            )
            ...
            fid_train.collect_features(bbox_real, label, padding_mask, real=True)
            ...
            if iteration % 50 == 0:
                ...
                print("iteration", iteration, last_eval)
            ...
            if iteration % 5000 == 0:
                ...
                if not out_path.exists():
                    save_image(bbox_real, label, mask, train_dataset.colors, out_path)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_dataloader):
                ...
                fid_val.collect_features(bbox_real, label, padding_mask, real=True)
